...
| `falcon-janitor` | container | Enforces the retention tiers (PLAN.md §13) over Falcon's own media directories. |
| `ntfy` | container | Self-hosted push with action buttons. |
| `falcon.db` | artifact | SQLite in WAL mode: events, decisions, audit trail. |
| `media/` | artifact | Crops, contact sheets, snapshots, clips — tiered retention (PLAN.md §13.3). |
| `media/pinned/` | artifact | Copied out of Frigate's managed storage on pin; never auto-deleted (§13.5–13.6). |
...
| `falcon-brain` | `media/pinned/` | file I/O | Copies pinned media out of Frigate's lifecycle (§13.6) |
